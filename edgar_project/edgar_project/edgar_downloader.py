...
def get_10k_artifact_links(symbol, params=None):
    ...
    headers = {'User-Agent': "magnus magnus@magnussfjdl.com"}#?action=getcompany&CIK=MSFT&count=100&type=10-k&output=atom'
    response     = requests.get(url_endpoint, q_params, headers=headers) #q_params
    ...
    if bool(soup.find(text="Your Request Originates from an Undeclared Automated Tool")):
        print("Automated web scraping error")
    print(filing_pages)
    return filing_pages
...
def get_10k_doc_links(filing_page_html, prefix=''):
    ...
    headers = {'User-Agent': "magnus magnus@magnussfjdl.com"}
