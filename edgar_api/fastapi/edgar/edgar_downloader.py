# pylint: disable=trailing-whitespace, line-too-long, fixme, invalid-name, broad-except, no-else-return, too-many-locals, unused-argument, unused-variable, missing-function-docstring
'''
Created: 2020-08-03
Author : Albert Tran
...
def get_10k_artifact_links(symbol, year, params=None):
    '''
    Arguments
    ---------
    ...
    headers = {'User-Agent': 'gibberish gibberish@nothing.com'}
    ...
    print(year)
    formatted_year = str(f'-{year[2]}{year[3]}-')
    for i in filing_pages:
        check = False
        if i.find(formatted_year) > 0:
            filing_pages = [i]
            check = True
            break
    if check is False:
        raise Exception(f"The year {year} is not valid for {symbol}")
    print(filing_pages)    
    return filing_pages
...
def download_files_10k(ticker, dest_folder, year): # added , year 
    '''
    Downloads 10-k files to directory.
    cNote that directory needs to already exist.
    '''
    filing_pages  = get_10k_artifact_links(ticker,year) # added , year
